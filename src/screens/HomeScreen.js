...
const HomeScreen = () => {
  ...
  const [singUpPhoto, setsingUpPhoto] = useState('');
    ...
    axios
      ...
      .then((response) => {
        if (response.data.registrationStatus === 'failed') {
          ...
          const ADD_VOTES_ENPOINT = 'http://localhost:5000/api/votes/add/';
          const userId = response.data.userId;
          const votes = { user_id: userId, score: 0 };

          dispatch({ type: 'REGISTER', payload: response.data.userId });

          axios
            .post(ADD_VOTES_ENPOINT, votes)
            .then((res) => console.log(res))
            .catch((err) => console.log(err));

          setSignupErrorMessage('User is registerred try to logg in!');
        }
      })
      .catch((err) => console.log(err));
  };
  ...
  return (
    <main>
      <div className='container'>
        <section>
          <h1 className='headline-1'>TEAM GAMES SIGN UP/LOGIN</h1>
        </section>
        ...
        <div className='login-signup-container'>
          ...
          <section id='signup' className='card-shadow'>
            ...
            <form id='signUpForm' className='form' onSubmit={signupUser}>
              ...
              <div className='form-control'>
                <label className='form-label' htmlFor='singUpPhoto'>
                  Photo URL
                </label>
                <input
                  className='form-input'
                  ...
                  required
                  value={singUpPhoto}
                  onChange={(e) => setsingUpPhoto(e.target.value)}
                />
              </div>
